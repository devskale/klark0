...
import { FileText, Search, Filter, CheckCircle, AlertCircle, FileCheck } from "lucide-react";
...
export default function ADoksPage() {
  const { selectedProject } = useProject();
  const [searchTerm, setSearchTerm] = useState("");
  const [filterType, setFilterType] = useState<string>("alle");
  const [filterAnforderung, setFilterAnforderung] = useState<string>("alle");

  const projektPath = selectedProject ? `${selectedProject}/projekt.json` : null;
  
  const { data: projektData, error, isLoading } = useSWR<ProjektData>(
    projektPath,
    fetcher,
  ...
  const filteredDokumente = dokumente.filter((dok) => {
    const matchesSearch = 
      ...
      (dok.beilage_nummer && dok.beilage_nummer.toLowerCase().includes(searchTerm.toLowerCase()));

    const matchesType = filterType === "alle" || dok.dokumenttyp === filterType;
    const matchesAnforderung = filterAnforderung === "alle" || dok.anforderungstyp === filterAnforderung;
  ...
  const anforderungstypen = [...new Set(dokumente.map(d => d.anforderungstyp))];

  const getAnforderungsBadge = (typ: string) => {
    switch (typ) {
      case "Pflichtdokument":
        return <Badge variant="destructive" className="text-xs">Pflicht</Badge>;
      case "Bedarfsfall":
        return <Badge variant="secondary" className="text-xs">Bedarfsfall</Badge>;
      default:
      ...
      <Card>
        <CardContent className="p-4">
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <Select value={filterAnforderung} onValueChange={setFilterAnforderung}>
              <SelectTrigger className="w-full sm:w-48">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-4">
            <div className="text-2xl font-bold text-red-600">
              {dokumente.filter(d => d.anforderungstyp === "Pflichtdokument").length}
            </div>
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-xs text-muted-foreground">Unterzeichnung erforderlich</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
          ...
          <Table>
            ...
            <TableBody>
              {filteredDokumente.map((dok, index) => (
                <TableRow key={index}>
                  <TableCell>
                    {getDokumenttypIcon(dok.dokumenttyp)}
                  </TableCell>
                  <TableCell>
                    <div>
