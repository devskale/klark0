...
- [x] **Kernfunktionen**: Projektmanagement, Dokumenten-Upload, WebDAV-Integration, Dokumentenkonvertierung (DOC/PDF zu Markdown)
  ...
  - [x] **WebDAV Root Path Filtering**: Comprehensive fix für WebDAV-Basispfad-Filterung in Ausschreibungen-Liste
    - [x] **Frontend Filtering**: Korrigierte Pfad-Vergleichslogik in auswahl/page.tsx (node.path vs. fileSystemConfig.basePath)
    - [x] **Backend Filtering**: Erweiterte Filterlogik in fileTreeFetcher für WebDAV-Pfadstrukturen (/disk/klark0/ vs. klark0)
    - [x] **Path Normalization**: Robuste Pfad-Normalisierung und Segment-Vergleich für verschiedene WebDAV-Server-Konfigurationen
    - [x] **Dynamic Base Path Implementation**: Vollständige Entfernung hardcodierter "klark0" Referenzen durch dynamische Basispfad-Utilities
      - [x] **API Routes**: Dynamische Basispfad-Erkennung in fs/, upload/, mkdir/ API-Routen basierend auf Einstellungen
      - [x] **Frontend Components**: Ersetzung hardcodierter Pfad-Replacements durch `extractProjectName()` und `removeWebDAVBasePath()` Utilities
      - [x] **Utility Functions**: Neue `getWebDAVBasePath()`, `extractProjectName()`, `removeWebDAVBasePath()` Funktionen für konsistente Pfad-Behandlung
      - [x] **Settings Integration**: Automatisches Laden der Filesystem-Einstellungen in Frontend-Komponenten für dynamische Basispfad-Verwendung
      - [x] **WebDAV URL Construction Fix**: Korrektur der URL-Konstruktion in allen API-Routen (fs/, upload/, mkdir/) zur Vermeidung von 405-Fehlern
        - [x] **Relative Path Handling**: Absolute Pfade werden zu relativen Pfaden konvertiert, um korrekte URL-Konstruktion mit `new URL()` zu gewährleisten
        - [x] **Host Path Preservation**: WebDAV-Host-Pfade (z.B. `/disk/`) werden nicht mehr durch Request-Pfade überschrieben
        - [x] **Consistent Logic**: Einheitliche Pfad-Override-Logik in allen drei API-Routen für konsistente Behandlung von Basispfad-Anfragen
